...
app.post("/login-request", (req, res) => {
    ...
    // Construct Railway domain login link
...
app.get("/posts/all", requireAuth, (req, res) => {
    ...
    `, [req.userPhone], (err, rows) => res.json(rows));
});
...
app.get("/search-api", requireAuth, (req, res) => {
    ...
    `, [req.userPhone, q, q, q], (err, rows) => res.json(rows));
});
...
async function extractInstagramMetadata(url) {
    try {
        ...
        return `${$('meta[property="og:title"]').attr("content") || ""} ${$('meta[property="og:description"]').attr("content") || ""}`;
    } catch {
        return null;
    }
}
...
app.post("/whatsapp", async (req, res) => {
    ...
    // NEW: If user sends LOGIN → generate link
    if (msg.trim().toUpperCase() === "LOGIN") {
        const token = crypto.randomBytes(30).toString("hex");
        db.run("INSERT INTO login_tokens (token, phone) VALUES (?, ?)", [token, from]);
...
`);
    ...
    db.get("SELECT phone FROM users WHERE phone = ?", [from], (err, row) => {
        if (!row) {
            db.run("INSERT INTO users (phone, name, avatar_color) VALUES (?, ?, ?)",
                [from, "User " + from.slice(-4), "mint"]);
        }
    });

    // ensure category
    db.get("SELECT id FROM main_categories WHERE name = ?", [main_category], (err, cat) => {
        if (!cat) {
            db.run("INSERT INTO main_categories (name) VALUES (?)", [main_category], function () {
    ...
    function handleLabel(catId) {
        db.get("SELECT id FROM labels WHERE name=? AND main_category_id=?",
            [label, catId],
            (err, lab) => {
                if (!lab) {
                    db.run("INSERT INTO labels (name, main_category_id) VALUES (?, ?)",
                        [label, catId],
                        function () { save(this.lastID); });
                } else save(lab.id);
            });
    }
